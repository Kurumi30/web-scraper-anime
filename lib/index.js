...
import { fetcher, getPublicationDate, replaceSpacesWithHyphens, sortedDate } from "./utils.js"
...
async function requestData(name) {
  ...
  try {
    ...
    return { links, animeTitle, englishTitle, description, type, imageUrl }
  } catch (err) {
    return
  }
}
...
export async function getEpisodeInfo(anime) {
  ...
  console.info({ result: args, data: sortedDate(uniqueData) })
  return { result: args, data: sortedDate(uniqueData) }
}

getEpisodeInfo("ore-dake-level-up-na-ken")